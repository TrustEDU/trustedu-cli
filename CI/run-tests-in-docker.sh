#!/bin/bash
#
# This script is run inside the Docker container and tests trustedu-cli
#
set -e
...
expect /opt/ci/test-trustedu-cli.expect

# Start neo-cli in background for additional JSON-RPC tests
screen -dmS node1 bash -c "dotnet TrustEDU.CLI.dll --rpc"
